    ...
    through2.obj(function z(file, encoding, next) {
      ...
      if (file.path.match(/(\/|\\)style(\/|\\)index\.js/)) {
        ...
        if (content.indexOf("'react-native'") !== -1) {
          // actually in qsq-mobile@2.0, this case will never run,
...
function publish(tagString, done) {
  let args = ['publish', '--with-qsq-tools', '--access=public'];
  if (tagString) {
  ...
  gulp.series(done => {
    ...
    if (npmArgs) {
      for (let arg = npmArgs.shift(); arg; arg = npmArgs.shift()) {
        if (/^pu(b(l(i(sh?)?)?)?)?$/.test(arg) && npmArgs.indexOf('--with-qsq-tools') < 0) {
          reportError();
          done(1);
          return;
        }
...
gulp.on('task_not_found', data => {
  console.log('task_not_found', data);
});

gulp.on('task_start', data => {
  console.log('task_start', data);
});

gulp.on('task_stop', data => {
  console.log('task_stop', data);
});
...
gulp.on('task_err', data => {
  console.log('task_err', data);
});
